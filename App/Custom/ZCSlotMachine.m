...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _backgroundImageView.backgroundColor=[UIColor colorWithRed:1 green:0.99 blue:0.92 alpha:1];
 ...
 //       _coverImageView.contentMode = UIViewContentModeCenter;
        ...
        //设置滚动的速度
        self.singleUnitDuration = 0.4f;
        
        _contentInset = UIEdgeInsetsMake(100, 100, 0, 0);
...
- (void)reloadData {
    if (self.dataSource) {
        ...
        for (int i = 0; i < numberOfSlots; i++) {
            ...
            if (kScreenWidth==320) {
                     slotContainerLayer.frame = CGRectMake(40+i * (slotWidth + slotSpacing), 0, slotWidth, _contentView.frame.size.height);
            }else if(kScreenWidth==375)
            {
                 slotContainerLayer.frame = CGRectMake(45+i * (slotWidth + slotSpacing), 0, slotWidth, _contentView.frame.size.height);
            }else{
                 slotContainerLayer.frame = CGRectMake(62+i * (slotWidth + slotSpacing), 0, slotWidth, _contentView.frame.size.height);
            }
        ...
        for (int i = 0; i < numberOfSlots; i++) {
            ...
            for (int j = 0; j > scrollLayerTopIndex; j--) {
                ...
                CGRect iconImageFrame=iconImageLayer.frame;
                if (kScreenWidth==320) {
                 iconImageFrame.origin.y= iconImageFrame.origin.y-28;
                }else if(kScreenWidth==375){
                 iconImageFrame.origin.y= iconImageFrame.origin.y-32;
                }
                else{
                 iconImageFrame.origin.y= iconImageFrame.origin.y-36;
                }
                iconImageLayer.frame=iconImageFrame;
